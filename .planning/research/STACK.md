...
**Project:** Expense Splitter — mobile-first bill-splitting web app
...
**Confidence:** React/TypeScript — HIGH (verified react.dev). Vite — MEDIUM (vitejs.dev was unreachable; version based on training data cross-checked with package ecosystem trajectory; current stable is 6.x as of early 2026).
...
**Do NOT use Zustand or Jotai for v1.** Both are excellent libraries, but they add a dependency and an abstraction layer that isn't earned by this app's complexity. The React docs explicitly recommend `useReducer` + Context as the right scaling step before reaching for external state management. Add Zustand only if the state model grows to multiple independent slices with non-trivial async logic (v2+ with persistence, history).

**Do NOT use Redux Toolkit.** Massive overkill for a no-backend calculation tool.

**Confidence:** HIGH — React 19 docs confirm this pattern as the idiomatic recommendation before external state managers.
...
**v4 vs v3 decision:** Use v4. The migration from v3 is breaking but the Vite-plugin path is cleaner than the old PostCSS setup, and v4 is the current release with active investment. Do not start new projects on v3.

**Do NOT use CSS Modules or styled-components.** Both require more boilerplate for responsive, touch-optimised UIs. Tailwind collocates styles with markup, which is exactly right for iterating quickly on mobile layouts.

**Confidence:** HIGH — verified on tailwindcss.com/blog and /docs/installation.
...
**Critical:** All money values stored and calculated as integers (cents). Display layer formats with `(cents / 100).toFixed(2)`. Rounding distribution (pennies that don't divide evenly) handled by assigning remainder to first/last person deterministically.

**Confidence:** HIGH — this is the standard pattern for money arithmetic in pure-JS apps.
...
**Confidence:** HIGH — both APIs are well-established browser standards present in all target environments (modern phone browsers).
...
**Confidence:** Vitest — MEDIUM (vitejs.dev unreachable; version from training data; 3.x is the current major line as of early 2026). @testing-library/react — HIGH (stable, widely documented).
...
**Confidence:** HIGH — both platforms officially support Vite static deployments.
